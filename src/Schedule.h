...
public:
    enum period_t : uint8_t {
        NONE, MINUTELY, HOURLY, WEEKLY, MONTHLY, YEARLY, ONCE
    }; // Частота повторов: нет, ежеминутно, ежечасно, еженедельно(ежедневно), ежемесячно, ежегодно, однократно
    ...
    Schedule(int8_t hh, int8_t mm, int8_t ss, uint8_t wd = 0B01111111) {
        set(hh, mm, ss, wd);
    } // Еженедельный(ежедневный) конструктор
    ...
    Schedule(int8_t hh, int8_t mm, int8_t ss, int8_t d, int8_t m, int16_t y) {
        set(hh, mm, ss, d, m, y);
    } // Однократный конструктор
    void clear() {
        _period = NONE;
        _nextTime = 0;
    } // Очистка расписания
    ...
    String toString(); // Строковое представление расписания
    operator String() { return toString(); }

    String nextTimeStr(); // Строковое представление времени следующего события
protected:
